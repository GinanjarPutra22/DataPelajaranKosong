...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($nama_ruang)) {
        ...
        $sql_check = "SELECT * FROM ruang WHERE nama_ruang = ?";
        if ($stmt_check = $conn->prepare($sql_check)) {
            $stmt_check->bind_param("s", $nama_ruang);
            $stmt_check->execute();
            $result_check = $stmt_check->get_result();

            if ($result_check->num_rows > 0) {
                $tambah_data_error = 'Nama Ruang sudah Ada!';
            } else {
                // Insert user baru
                $sql_insert = "INSERT INTO ruang (nama_ruang) VALUES (?)";
                if ($stmt_insert = $conn->prepare($sql_insert)) {
                    $stmt_insert->bind_param("s", $nama_ruang);
...
// Ambil data pengguna
$sql_select = "SELECT * FROM ruang ORDER BY nama_ruang";
...
<?php 
    ...
    <div class="container pt-5 pb-5">
        <div class="d-flex align-items-center mb-4">
            ...
            <h2 class="text-center m-0 flex-grow-1">Data Ruang</h2>
        </div>
        <div class="card">
            <div class="card-body">
                <form method="post" action="ruang.php">
                    <div class="mb-3">
                        ...
                        <input type="text" id="nama_ruang" name="nama_ruang" class="form-control" required>
                    </div>
                ...
                <tbody class="table-group-divider"> 
                    ...
                    while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td>
                                <a href="ruang.php?hapus_ruang=<?= $row['id_ruang'] ?>" class="btn btn-danger" onclick="return confirm('Yakin ingin menghapus data ini?')">Hapus</a>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                </tbody>
